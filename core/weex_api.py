...
def _post(path, data):
    ...
    for _ in range(RETRIES + 1):
        ...
        try:
            ...
            # Always attempt to parse JSON for logging
            try:
                parsed = res.json()
            except Exception:
                parsed = None

            # 🔎 NEW: Log WEEX JSON response always
            log.error(f"[WEEX RESPONSE] status={res.status_code} body={parsed or res.text}")
...
def get_all_positions():
    ...
    if not res:
        log.error("GET POSITIONS FAILED")
        return []
    ...
    for p in res:
        try:
            ...
            # Only LONG (you trade long only)
            if p.get("side") != "LONG":
                continue

            open_value = float(p.get("open_value", 0))
            contract_val = float(p.get("contractVal", 1))

            if size != 0 and contract_val != 0:
                entry_price = open_value / (size * contract_val)
            else:
                entry_price = 0
